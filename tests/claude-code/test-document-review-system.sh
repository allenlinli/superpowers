...
mkdir -p docs/spsa/changes/test-feature

# Create a spec document WITH INTENTIONAL ERRORS for the reviewer to catch
cat > docs/spsa/changes/test-feature/design.md <<'EOF'
# Test Feature Design
...
Then review the spec at $TEST_PROJECT/docs/spsa/changes/test-feature/design.md using the criteria from that template.
